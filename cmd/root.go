...
func Execute() error {
	...
	if err != nil {
		printCommandError(rootCmd, executedCmd, err)
	}
	return err
}

func printCommandError(root, executedCmd *cobra.Command, err error) {
	_, _ = root.ErrOrStderr().Write([]byte(err.Error() + "\n"))
	if isUsageError(err) {
		usageCmd := usageCommand(root, executedCmd)
		_, _ = root.ErrOrStderr().Write([]byte("\n" + usageCmd.UsageString()))
	}
}

func usageCommand(root, executedCmd *cobra.Command) *cobra.Command {
	if executedCmd != nil {
		return executedCmd
	}
	return root
...
func registerRootFlags(cmd *cobra.Command) {
	cmd.PersistentFlags().String("config", "", "path to Prolfile.toml (default: auto-discover upward)")
	cmd.PersistentFlags().String("runtime", "", "override runtime: swi | gnu | scryer")
	cmd.PersistentFlags().Bool("verbose", false, "enable verbose/debug output")
	cmd.PersistentFlags().Bool("no-color", false, "disable colour output (also respects NO_COLOR env var)")
